...
1. Read `README.md`, `THEORIE.md`, `AGENTS.md`, `VISION.md` — do any of these exist already?
...
2. **What is the current goal?** What should Praxis work on first? What's the vision for the next weeks/months?
...
5. **Are there areas Praxis should NOT touch?** Protected files, stable modules, external integrations.
6. **Which agents do you want to use?** Configure `PRAXIS_AGENT` / `THEORIE_AGENT` in `config.sh`.
...
This is what Praxis reads to plan features. It should contain:
...
Keep it short and actionable. Praxis reads this every iteration — brevity matters.
...
### THEORIE.md (in repo root)
The Theorie role definition. Start with this template and adapt (replace `<theorie-label>` and `<praxis-label>` with values from config.sh):

```markdown
# THEORIE.md

## Role
The Theorie loop reviews plans/implementations, fixes bugs, and evaluates feedback. Theorie does NOT implement features.

## Routing
Theorie acts when a plan or bug is `assigned-to: <theorie-label>`:
...
- `open` + `assigned-to: <theorie-label>` → Fix bug, commit, set Status: fixed

Feedback (in `.workflow/feedback/`):
- Files without `## Theorie Assessment` → Evaluate and incorporate

## What Theorie does NOT do
- Implement features
- Change plan status (Praxis does that, except: Theorie may set ci-blocked → awaiting-implementation-review after fixing)
```

### .workflow/config.sh
Update with the correct values:
- `PRAXIS_AGENT` / `THEORIE_AGENT` → which ralph agents to use
...
- `THEORIE_FILE` → path to THEORIE.md
...
1. Confirm all files exist: `VISION.md`, `AGENTS.md`, `THEORIE.md`, `.workflow/config.sh`
...
- Keep VISION.md under 50 lines — Praxis reads it every iteration
